...
const loadMusicError = (error) => {
  ...
  const currentIndex = aplayer?.index ?? 0;
  const currentAudio = playList.value[currentIndex];
  const songName = currentAudio?.name ?? "未知歌曲";
  
  console.error(
    `播放歌曲：${songName} 出现错误`,
    error || aplayer?.audio?.[currentIndex],
  );
};
